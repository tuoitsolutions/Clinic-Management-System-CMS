...
{
    public class ConsultVitalSignRepo
    {

        public ResponseModel InsertConsultVitalSign(ConsultVitalSignEntity payload)
        {
            try
            {
                ...
                int add_dept = con.Execute(@"
                        INSERT INTO `consult_req_vital_sign` SET
        ...
        public ResponseModel UpdateConsultVitalSign(ConsultVitalSignEntity payload)
        {
            try
            {
                using var con = new MySqlConnection(DatabaseConfig.GetConnection());
                ...
                int edit_dept = con.Execute(@"
                        UPDATE `consult_req_vital_sign` SET
                        blood_pressure=@blood_pressure,
                        heart_rate=@heart_rate,
                        resp_rate=@resp_rate,
                        temperature=@temperature,
                ...
                if (edit_dept > 0)
                {
                    ...
                    {
                        activity = $"The vital sign with ID {payload.cr_vs_pk} has been updated!",
                        encoded_by = payload.encoder_pk,
                        ref_pk = payload.cr_vs_pk.ToString(),
                        ref_table = "consult_req_vital_sign"
                    };
        ...
        public ResponseModel GetTableConsultVitalSign(ConsultVitalSignTablePayload payload)
        {
            try
            {
                using var con = new MySqlConnection(DatabaseConfig.GetConnection());
                con.Open();
                using var tran = con.BeginTransaction();
                List<ConsultVitalSignEntity> table_data = con.Query<ConsultVitalSignEntity>($@"
                                       SELECT * FROM (
                                        SELECT * FROM `consult_req_vital_sign`
                                        ) AS view_tmp
                                       WHERE
                                       `consult_req_pk` = @consult_req_pk
        ...
        public ResponseModel GetConsultVitalSignByPk(string id)
        {
            try
            {
                using var con = new MySqlConnection(DatabaseConfig.GetConnection());
                ...
                List<ConsultVitalSignEntity> table_data = con.Query<ConsultVitalSignEntity>(
                    $@"SELECT * FROM consult_req_vital_sign where cr_vs_pk=@id limit 1;",
                    new { id }, transaction: tran).ToList();

                if (table_data.Count > 0)
                {
                    ConsultVitalSignEntity selected_admin = table_data[0];
