...
{
    public class ConsultMedProbRepo
    {

        public ResponseModel InsertConsultMedProb(ConsultMedProbEntity payload)
        {
            try
            {
                ...
                int add_dept = con.Execute(@"
                        INSERT INTO `consult_req_med_prob` SET
                        consult_req_pk=@consult_req_pk,
                        med_prob_pk=@med_prob_pk,
                        med_prob_desc=@med_prob_desc,
                        med_prob_host=@med_prob_host,
                        is_active=@is_active,
                        encoded_at=NOW(),
        ...
        public ResponseModel UpdateConsultMedProb(ConsultMedProbEntity payload)
        {
            try
            {
                using var con = new MySqlConnection(DatabaseConfig.GetConnection());
                ...
                int edit_dept = con.Execute(@"
                        UPDATE `consult_req_med_prob` SET
                        med_prob_pk=@med_prob_pk,
                        med_prob_desc=@med_prob_desc,
                        med_prob_host=@med_prob_host,
                        is_active=@is_active,
                        encoded_at=NOW(),
                        encoder_pk=@encoder_pk
                        WHERE cr_med_prob_pk=@cr_med_prob_pk;
                        ", payload, transaction: tran);
                ...
                if (edit_dept > 0)
                {
                    ...
                    {
                        activity = $"The vital sign with ID {payload.cr_med_prob_pk} has been updated!",
                        encoded_by = payload.encoder_pk,
                        ref_pk = payload.cr_med_prob_pk.ToString(),
                        ref_table = "consult_req_med_prob"
                    };
        ...
        public ResponseModel GetTableConsultMedProb(ConsultMedProbTablePayload payload)
        {
            try
            {
                using var con = new MySqlConnection(DatabaseConfig.GetConnection());
                con.Open();
                using var tran = con.BeginTransaction();
                List<ConsultMedProbEntity> table_data = con.Query<ConsultMedProbEntity>($@"
                                       SELECT * FROM (
                                        SELECT * FROM `consult_req_med_prob`
                                        ) AS view_tmp
                                       WHERE
                                       `consult_req_pk` = @consult_req_pk
                                       AND COALESCE(med_prob_desc,'') LIKE CONCAT('%',@med_prob_desc,'%')
                                       AND med_prob_host IN @med_prob_host
        ...
        public ResponseModel GetConsultMedProbByPk(string id)
        {
            try
            {
                using var con = new MySqlConnection(DatabaseConfig.GetConnection());
                ...
                List<ConsultMedProbEntity> table_data = con.Query<ConsultMedProbEntity>(
                    $@"SELECT * FROM consult_req_med_prob where cr_med_prob_pk=@id limit 1;",
                    new { id }, transaction: tran).ToList();

                if (table_data.Count > 0)
                {
                    ConsultMedProbEntity selected_admin = table_data[0];
