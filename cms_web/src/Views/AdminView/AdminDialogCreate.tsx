  ...
  const handleSubmitForm = useCallback(
    async (data: AdminEntity) => {
      ...
      dispatch(
        setGeneralPrompt({
          open: true,
          custom_title: `Are you sure that you want to add this admnistrator?`,
          continue_callback: async () => {
            dispatch(
              showPageLoading({
                show: true,
                loading_message:
                  "Adding admnistrator, thank you for your patience",
              })
            );

            const response = await AdminApi.InsertAdmin(form_data_payload);
