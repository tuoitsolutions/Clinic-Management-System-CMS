...
import ConsultMedProbEntity, {
  ConsultMedProbTableModel,
} from "../../../Services/Entities/ConsultMedProbEntity";
import { PaginationModel } from "../../../Services/Models/PaginationModel";
import {
  TblColumnModel,
  TblInitialSortModel,
} from "../../../Services/Models/TableModels";
import DialogAddMedProb from "./DialogAddMedProb";
import DialogUpdateMedProb from "./DialogUpdateMedProb";

interface ITabMedProbRecord {
...
const init_tbl_sort: Array<TblInitialSortModel> = [
  ...
  //
  {
    label: "Medical Problem (asc)",
    value: {
      column: "med_prob_desc",
      direction: "asc",
    },
  },
  {
    label: "Medical Problem (desc)",
    value: {
      column: "med_prob_desc",
      direction: "desc",
    },
...
export const TabMedProbRecord: FC<ITabMedProbRecord> = memo(
  ({ consult_req_pk }) => {
    ...
    const [data_table, set_data_table] =
      useState<null | ConsultMedProbTableModel>(null);
    ...
    const [selected_record, set_selected_record] =
      useState<null | ConsultMedProbEntity>(null);
    ...
    useEffect(() => {
      ...
      const fetchTableData = async () => {
        ...
        const table_response = await ConsultMedProbApi.GetTableConsultMedProb(
          filters
        );
                    ...
                    <TableBody>
                      ...
                      {data_table?.table?.map((row, index) => (
                        <TableRow key={index}>
                          ...
                          <TableCell>{row.med_prob_desc}</TableCell>
...
export default TabMedProbRecord;
